...
/* Utilisation*/
// ./client <pseudo-file-client> <pseudo-file-server>
...
int main(int argc, char * argv[]) {
    ...
    int pid = (int) getpid();
    struct sockaddr_un adrcli;  // adresse de la socket client
    ...
    CHECK_ERROR(bind(sd, (const struct sockaddr *) &adrcli, sizeof(adrcli.sun_family) + strlen(adrcli.sun_path)), -1, "Échec du bind de la socket du client !");

    // Étape 4 : Remplir l'adresse de la socket du serveur
    adrserv.sun_family = AF_UNIX;
    if (argc > 2) strcpy(adrserv.sun_path, argv[2]);
    else          strcpy(adrserv.sun_path, "SocketServ");
